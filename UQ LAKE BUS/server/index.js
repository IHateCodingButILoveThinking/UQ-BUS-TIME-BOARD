...
export async function fetchDepartures() {
  ...
  const departures = stopTimetables
    ...
    .sort((left, right) => {
      return new Date(left.scheduledUtc).getTime() - new Date(right.scheduledUtc).getTime();
    })
    .slice(0, 24);
  ...
  const payload = {
    ...
    sourceUrl: "https://jp.translink.com.au/plan-your-journey/stops/uq%20lakes%20station",
    departures,
  };
  ...
  return departures
    ...
    .map((departure) => {
      const route = routes.find(
        (candidate) =>
          candidate.id === departure.routeId && candidate.direction === departure.direction,
      );
      const scheduledUtc =
    ...
    .filter((departure) => new Date(departure.scheduledUtc).getTime() >= now.getTime() - 60_000);
}
